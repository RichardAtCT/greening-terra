...
func setup(def: ResourceNodeDef, pos: Vector2, rng: RandomNumberGenerator) -> void:
	...
	for i in 3:
		...
		mi.position = Vector3(cos(a) * 0.45, 0.35 + i * 0.1, sin(a) * 0.45)
		mi.rotation = Vector3(rng.randf() * 3, rng.randf() * 3, rng.randf() * 3)
		mi.scale = Vector3(1, 1.5 if crystal else 0.8, 1)
		add_child(mi)
